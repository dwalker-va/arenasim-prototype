#!/bin/bash
#
# Shaman 2v2 Matrix Sweep
#
# Runs N matches per matchup for Shaman+Priest vs each-class+Priest (6 matchups)
# and aggregates winrates into a CSV byte-compatible with the 1v1 matrix output
# from src/headless/matrix.rs:217.
#
# Usage:
#   ./scripts/shaman_2v2_matrix.sh [N] [--seed-base SEED] [--out OUT_CSV] [--swap-sides]
#
# Arguments:
#   N             Number of seeds per matchup (default: 100)
#   --seed-base   Base RNG seed; each match gets seed = base + run_idx (default: 0)
#   --out         Output CSV path (default: match_logs/shaman_2v2_<timestamp>.csv)
#   --swap-sides  Run the transposed ordering (<class>+Priest as team1, Shaman+Priest
...
if [[ -z "$OUT_CSV" ]]; then
    mkdir -p match_logs
    OUT_CSV="match_logs/shaman_2v2_${TIMESTAMP}.csv"
fi
...
TEMP_DIR=$(mktemp -d -t shaman_2v2_XXXXXX)
trap 'rm -rf "$TEMP_DIR"' EXIT

# Shaman+Priest vs each opposing class + Priest (mirror healer partner)
HEALER="Priest"
OPPONENTS=("Warrior" "Mage" "Rogue" "Priest" "Warlock" "Paladin" "Hunter")
...
if [[ "$SWAP_SIDES" -eq 1 ]]; then
    echo "Running <class>+${HEALER} vs Shaman+${HEALER} 2v2 sweep (swapped sides): N=${N} per matchup, seed_base=${SEED_BASE}"
else
    echo "Running Shaman+${HEALER} vs <class>+${HEALER} 2v2 sweep: N=${N} per matchup, seed_base=${SEED_BASE}"
fi
echo "Output: $OUT_CSV"
echo ""
...
for opp in "${OPPONENTS[@]}"; do
    ...
    SIDE_A="Shaman"; SIDE_B="${opp}"
    if [[ "$SWAP_SIDES" -eq 1 ]]; then SIDE_A="${opp}"; SIDE_B="Shaman"; fi
