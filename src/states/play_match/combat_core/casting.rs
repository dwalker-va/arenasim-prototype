...
pub fn process_casting(
    ...
    for (caster_entity, caster_team, caster_slot, caster_class, caster_pos, ability_damage, ability_healing, ability, target_entity, is_crit_damage, is_crit_heal, caster_spell_power, mana_cost) in completed_casts {
        ...
        if def.is_damage() {
            ...
            // Deterministic spawn — NO game_rng draw, unlike the Immolate block
            // above — so this is byte-neutral in headless. The forked-arc geometry
            // and flash/impact burst are generated by the graphical-only systems in
            // rendering/effects/lightning_bolt.rs, which never run in headless.
            if ability == AbilityType::LightningBolt {
                commands.spawn((
                    LightningBoltStrike {
                        start: caster_pos,
                        end: target_pos,
