...
- Sandbox scene gating and teardown: src/states/mod.rs:63,223 (`in_combat_scene` as shared scene condition — movement runs in the sandbox; the sheep wanders on stage) and src/states/animation_sandbox/playback.rs:410-430, 528-545 (`clear_body_state` aura-clear restore; `sustain_staged_units` heals rather than damages, so staged polymorphs never insta-break).
