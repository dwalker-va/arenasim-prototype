...
- KTD2. **`add_core_combat_systems` gains two run conditions — one registration, two gateable layers.** It now takes a `scene_condition` (where combat RESOLUTION runs) and a `decision_condition` (where the AI and match clock ALSO run), applied as a per-system `run_if` inside the existing `.chain()`. Graphical passes `in_combat_scene` and `in_state(PlayMatch)`; headless passes always-true for both, so every system runs as before.

  A second registration of the resolution systems for the sandbox was tried first and **does not work**: it makes their `SystemTypeSet`s ambiguous, and Bevy then rejects any `.after()`/`.before()` naming one. `spawn_projectile_visuals` orders against `process_channeling` and `move_projectiles`, so the app panicked at schedule init on entering the sandbox. Schedule ambiguity is global, so no per-state gating avoids it. Only `cargo test` plus a graphical smoke boot caught this — the unit tests all passed.
