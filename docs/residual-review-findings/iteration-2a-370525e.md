...
The single load-bearing critique across maintainability (#5 and #6 in the synthesis) is that this iteration lands ~50 LOC of inert infrastructure (PetCommand component, apply_deferred wiring, `ability_cooldowns` snapshot field, `start_pet_dispatch_decision` builder, AbilityType Ord derive) whose only consumer is iteration 2b's U4. This was an explicit user decision at the U2 commit boundary mid-LFG ("keep going — push through U1-U8 + validation in one session"), but the autopilot ultimately delivered U1+U2+U3+U5+U6+U7+U8 and deferred U4. The honest read: the dead-code findings are acceptable IF iteration 2b lands U4 promptly. If U4 slips, this commit should be revisited — either revert U3 (PetCommand + EventPayload extension + LowHealthHeel + apply_deferred) or land U4 standalone before this branch merges.
