...
`animate_body_lean` (`src/states/play_match/rendering/effects/weapon_swing.rs`) lifted it for the whole melee tier at once. The body turns about the swing's **own** axis ([`arc_rotation`]) by `SwingProfile::lean` of its angle, driven by the same `s` the weapon uses — so wind-back and drive-through fall out of one input, with no second curve to keep in sync. Three things make it work:
...
**Amendment, A2 (2026-08-28) — research the source for EVERY ability, not just the one you feel unsure about.** Four caster-side gestures were built together, and reading the actual Classic client data (DB2 tables, then the parsed M2 models and BLP textures) reversed the design for three of them. Kidney Shot is a lunging `Attack1HPierce`, **not** the kick everyone remembers — the kick is the Rogue's *Kick*. Cheap Shot is so generic it shares its visual with Sap and has zero colour tracks. Hammer of Justice has **no hammer at all**: `HasMissile = 0`, and it is internally named *FistOfJustice*. Note the second-order lesson here: `HasMissile = 0` rules out a *projectile*, and the first design over-read it as ruling out aimed motion too, producing a gold streak thrown at the victim. Going back to the reference imagery showed a ring — the shipped effect is a caster-centred expanding wave, and the caster keeps a raised-weapon stroke. Every one of those was something the model had asserted confidently from memory first.
...
**Amendment, the shared impact (2026-09-01) — the recolour tier exists, and the research reshaped it again.** Roadmap section D built the third generic hook: `SchoolImpact` (`rendering/effects/school_impact.rs`), one marker spawned at the two resolution sites and one per-school row. Four lessons:

- **Research every ability, even the ones that "obviously" share a look.** In the client data Aimed Shot, Arcane Shot and Concussive Shot literally share one impact model, which confirmed the shared tier — but Mind Blast turned out to be a two-second smoulder on the HEAD, not a chest burst, and Web has no impact kit at all (only the root state). One anchor enum and one `None` arm came straight out of that, and the roadmap's list of "four silent projectiles" was wrong by one in each direction.
- **A school colour is not automatically an effect colour.** `SpellSchool::Physical`'s tan is within 0.07 of the arena floor in RGB; an additive tan burst is invisible on sand. Physical went hueless, carrying its identity in material and motion (lit splinters that fall and splash back). Check any new school row against `create_surface_texture`'s base colour before trusting the authority.
- **Bench the unit the way Bevy places it.** The bolt bench drew the capsule with its feet at the transform; Bevy centres it there. Every anchor measured off that bench was 1.25yd too high, and the bolt impacts shipped 0.2yd above the head — a defect that passed 23 geometry probes, because the probes asserted against the same wrong constant. A probe that pins an anchor must pin it against the capsule's real extent, which `the_anchors_sit_on_the_capsule` now does.
- **"Spawning a marker is byte-identical" is a per-comp claim, and the recorded baseline may be stale.** The behaviour-baseline diff against `tests/baselines/legacy_behaviour_2026-08-02_backlash_ids.txt` showed all nine pet-comp cells moved; main itself has the same nine moved — the file predates later Hunter/Shaman work. Diff a branch against a fresh run of main's commit, not only against the recorded file. Separately, `combat_ai.rs` still breaks target-score ties by `Entity::index()`, so any change to entity lifetimes could in principle move a tie; it did not here, but it is the one place a cosmetic spawn can leak into the sim.

**The receiver side does not have to be a body treatment.** Mortal Wounds — a 10s `HealingReduction` debuff — gets no visual on the victim at all. It states itself by breaking incoming heals: a heal landing on an afflicted target sheds the refused share as ash (`src/states/play_match/rendering/effects/mortal_wounds.rs`), keyed at the three sites that already apply the reduction. This is cheaper than a body treatment AND more legible, because it fires exactly when the debuff costs someone something — and it sidesteps the whole `OriginalBodyMaterial` contention family (see `shared-restore-slot-mutual-exclusion.md`) by never touching the body. Prefer it whenever a debuff's meaning is "some other mechanic is now worse."
...
7. **Pin the restore contract with headless probes** (`tests/polymorph_visual_probes.rs` is the template: `MinimalPlugins` + `AssetPlugin`, manual clock, systems added directly). Transform-in, non-accumulation across repeats, every exit path, and owner scoping are all cheap assertions. See `aura-driven-visual-exit-paths.md` for the exit paths that MUST be covered. **APPEARANCE is untestable; GEOMETRY is not** — an earlier wording of this step said appearance was untestable and stopped there, and that belief is precisely what let four A2 defects ship past a green suite (a decal yawed 90° off, one reaching half as far as its own `length` field claimed, a fan clumped at a single point, and slashes running head-to-toe). Orientation, world extent, and on-screen readability are all one-line assertions. See `visual-probes-assert-rendered-geometry.md` — the geometry half of what a visual probe owes.
...
- Instant-ability flourishes hang off `InstantAbilityFired` (renamed from
  ...
  resolution site and dispatched by `src/states/play_match/rendering/effects/instant_ability.rs`. The
...
The Polymorph pilot itself: `update_polymorph_visuals` + `spawn_sheep_parts` (`src/states/play_match/rendering/effects/polymorph.rs`), `update_sheep_hop` (`src/states/play_match/rendering/effects/gait.rs`), and the transform-puff trio (`src/states/play_match/rendering/effects/transform_puffs.rs`); probes in `tests/polymorph_visual_probes.rs`, sandbox staging in `src/states/animation_sandbox/playback.rs`.
