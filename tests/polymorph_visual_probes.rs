...
impl Harness {
    ...
    fn spawn_unit(
        ...
        team: u8,
        slot: u8,
    ) -> (Entity, Entity, Handle<Mesh>, Handle<StandardMaterial>) {
        let mesh = self
            .app
            .world_mut()
...
fn transforms_and_restores() {
    ...
    assert_ne!(
        h.app.world().get::<Mesh3d>(body).unwrap().0,
        original_mesh,
        "torso swapped"
    );
    assert_ne!(
    ...
    assert!(
        parts >= 8,
        "expected head/muzzle/ears/legs/tail, got {parts}"
    );
...
fn hop_lifts_a_moving_sheep_and_settles_a_still_one() {
    ...
    assert!(
        peak > 0.15,
        "hop should clear the walk bob's 0.10, got {peak}"
    );
    ...
    assert!(
        (walk.previous_xz.x - 6.0 * 0.225).abs() < 1e-4,
        "baseline left stale"
    );
    ...
    assert!(
        resting.abs() < 0.01,
        "a stationary sheep should settle to rest, got {resting}"
    );
}
...
fn sheep_stands_on_the_floor() {
    ...
    assert!(
        (lowest - 0.30).abs() < 0.01,
        "legs should stand on the floor, got {lowest}"
    );
    assert!(
        highest < 1.0,
        "sheep should stay well under the capsule top, got {highest}"
    );
}
